...
def get_next_package_release(versions_json):
    ...
    try:
        ...
        print(
            f"No published release for DRM library version {version}, "
            'setting next release number to "1"'
        )
        return 1
    next_release = release + 1
    print(
        f"DRM library version {version} was already published with release "
...
def publish_packages(
    pkg_source, versions_json, deb_repo, rpm_repo, deb_s3, gpg_private_key, gpg_key_id
):
    """
    ...
    rpm_conf = {
        ...
        "baseurl": f"{repo_base_url}/rpm/{component}/$releasever/$basearch/",
        "enabled": "1",
    ...
    run(["gpg", "--batch", "--no-tty", "--import", gpg_private_key], **run_kwargs)
    ...
    for root, dirs, files in walk(pkg_source):
        for file_name in files:
            ...
            # <name>-<version>-<release>.<os_release>.<cpu_arch>.rpm
                ...
                packages["rpm"][releasever][basearch].append((pkg_path, file_name))
                print(
                    f'found RPM (releasever="{releasever}", '
    ...
    with TemporaryDirectory() as base_dir:
        ...
        for codename, pkg_set in packages["deb"].items():
            for pkg_path in pkg_set:
                run(
                    reprepro
                    + ["--component", component, "includedeb", codename, pkg_path],
                    **run_kwargs,
                )
