...
def _init_gpg_configuration(private_key=None, quiet=False):
    ...
    # Import private key
    _run(
        ["gpg", "--batch", "--no-tty", "--import", private_key],
        quiet=quiet,
...
def sign_rpm_packages(
    packages_dir, private_key=None, public_key=None, pass_phrase=None, quiet=False
):
    """
    ...
    packages = [
        package
        for package in _listdir(packages_dir)
        if _splitext(package)[1].lower() == ".rpm"
    ]

    gpg_info = _run("gpg --export | gpg --list-packets", shell=True, quiet=True).stdout
    ...
    _run(["rpm", "--addsign"] + define + packages, quiet=True, cwd=packages_dir)

    # Verify signatures
    result = _run(["rpm", "--checksig"] + packages, quiet=True, cwd=packages_dir)
    for line in result.stdout.splitlines():
        line = line.rstrip()
        if (
            not line.endswith("gpg OK")
            and not line.endswith("pgp md5 OK")
            and not line.endswith("digests signatures OK")
        ):
            raise RuntimeError("Package signature verification failure: %s" % line)
...
def sign_deb_packages(
    ...
    _run(command + ["--sign", "builder"] + packages, quiet=quiet, cwd=packages_dir)
...
if __name__ == "__main__":
    ...
    def run_command():
        ...
        parser = ArgumentParser(
            prog="sign_packages", description="Sign RPM or DEB packages."
        )
