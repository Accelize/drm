...
# else
    ...
    #define checkDRMCtlrRet( func ) {                                                  \
        ...
        try {                                                                          \
            ...
            Throw( DRM_CtlrError, spdlog::level::err, e.what() );                    \
        }                                                                              \
        if ( errcode )                                                                 \
        ...
        try {
            ...
            if ( param_lib != Json::nullValue ) {
                ...
                if ( mWSRequestTimeout == 0 )
                    Throw( DRM_BadArg, spdlog::level::err, "ws_request_timeout must not be 0");
            }
            mHealthPeriod = 0;
            ...
            if ( mWSRetryPeriodLong <= mWSRetryPeriodShort )
                Throw( DRM_BadArg, spdlog::level::err, "ws_retry_period_long ({}) must be greater than ws_retry_period_short ({})",
            ...
            if ( e.getErrCode() != DRM_BadFormat )
                throw;
            Throw( DRM_BadFormat, spdlog::level::err, "Error in configuration file '{}: {}", conf_file_path, e.what() );
        }
    }
    ...
    void createFileLog( const std::string file_path, const eLogFileType type,
        ...
        if ( type == eLogFileType::NONE )
            ...
            if ( !makeDirs( parentDir ) ) {
                Throw( DRM_ExternFail, spdlog::level::err, "Failed to create log file {}", file_path );
            }
    ...
    std::vector<uint32_t> readMailbox( const eMailboxOffset offset, const uint32_t& nb_elements ) const {
        ...
        if ( index + nb_elements > rwData.size() )
            Throw( DRM_BadArg, spdlog::level::err, "Trying to read out of Mailbox memory space; size is {}", rwData.size() );
    ...
    void writeMailbox( const eMailboxOffset offset, const std::vector<uint32_t> &value_vec ) const {
        ...
        if ( index + value_vec.size() > rwData.size() )
            Throw( DRM_BadArg, spdlog::level::err, "Trying to write out of Mailbox memory space: {}", rwData.size() );
    ...
    void lockDrmToInstance() {
        ...
        if ( isLocked )
            Throw( DRM_BadUsage, spdlog::level::err, "Another instance of the DRM Manager is currently owning the HW" );
    ...
    void checkHdkCompatibility() {
        ...
        if ( drmMajor < HDK_COMPATIBILITY_LIMIT_MAJOR ) {
            Throw( DRM_CtlrError, spdlog::level::err,
    ...
    void runBistLevel1() const {
        ...
        for(unsigned int i=0; i<=5; i++) {
            if ( writeDrmRegister( "DrmPageRegister", i ) != 0 )
                Throw( DRM_BadArg, spdlog::level::err, "DRM Communication Self-Test 1 failed: Could not write DRM page register\n{}", DRM_SELF_TEST_ERROR_MESSAGE ); //LCOV_EXCL_LINE
            if ( readDrmRegister( "DrmPageRegister", reg ) != 0 )
                Throw( DRM_BadArg, spdlog::level::err, "DRM Communication Self-Test 1 failed: Could not read DRM page register\n{}", DRM_SELF_TEST_ERROR_MESSAGE ); //LCOV_EXCL_LINE
            if ( reg != i ) {
                Throw( DRM_BadArg, spdlog::level::err, "DRM Communication Self-Test 1 failed: Could not switch DRM register page.\n{}", DRM_SELF_TEST_ERROR_MESSAGE ); //LCOV_EXCL_LINE
            }
        }
    ...
    void runBistLevel2() const {
        ...
        if ( mbSize >= 0x10000 ) {
            ...
            Throw( DRM_BadArg, spdlog::level::err, "DRM Communication Self-Test 2 failed: Unexpected mailbox size ({} > 0x10000).\n{}", mbSize, DRM_SELF_TEST_ERROR_MESSAGE); //LCOV_EXCL_LINE
        }
        ...
        if ( badData.size() ) {
            ...
            Throw( DRM_BadArg, spdlog::level::err, "DRM Communication Self-Test 2 failed: all 0 test failed.\n{}", DRM_SELF_TEST_ERROR_MESSAGE); //LCOV_EXCL_LINE
        }
        ...
        if ( badData.size() ) {
            ...
            Throw( DRM_BadArg, spdlog::level::err, "DRM Communication Self-Test 2 failed: all 1 test failed.\n{}", DRM_SELF_TEST_ERROR_MESSAGE); //LCOV_EXCL_LINE
        }
        ...
        if ( badData.size() ) {
            ...
            Throw( DRM_BadArg, spdlog::level::err, "DRM Communication Self-Test 2 failed: random test failed.\n{}", DRM_SELF_TEST_ERROR_MESSAGE); //LCOV_EXCL_LINE
        }
    ...
    void initDrmInterface() {
        ...
        try {
            ...
            if ( err_msg.find( "Unable to select a register strategy that is compatible with the DRM Controller" )
                    != std::string::npos )
                Throw( DRM_CtlrError, spdlog::level::err, "Unable to find DRM Controller registers.\n{}", DRM_SELF_TEST_ERROR_MESSAGE );
            Throw( DRM_CtlrError, spdlog::level::err, "Failed to initialize DRM Controller: {}", e.what() );
        }
        ...
        if ( isNodeLockedMode() ) {
            ...
            if ( !isDir( mNodeLockLicenseDirPath ) )
                Throw( DRM_BadArg, spdlog::level::err,
    ...
    Json::Value getMeteringHeader() {
        ...
        if ( !mailboxReadOnly.empty() ) {
            try {
                ...
                if ( e.getErrCode() == DRM_BadFormat )
                    Throw( DRM_BadFormat, spdlog::level::err, "Failed to parse Read-Only Mailbox in DRM Controller: {}", e.what() );
                throw;
            }
        } else {
    ...
    uint64_t getMeteringDataInt64() const {
        ...
        if  ( meteringFileStr.size() ) {
            ...
            if ( errno ) {
                Throw( DRM_CtlrError, spdlog::level::err, "Could not convert string '{}' to unsigned long long.",
                       meteringDataStr );
            }
        }
        return meteringData;
        ...
        while ( 1 ) {
            ...
            try {
                ...
                if ( e.getErrCode() == DRM_WSTimedOut ) {
                    // Reached timeout
                    Throw( e.getErrCode(), spdlog::level::err, "Timeout on Authentication request after {} attempts", oauth_attempt );
                }
            ...
            try {
                ...
                if ( e.getErrCode() == DRM_WSTimedOut ) {
                    // Reached timeout
                    Throw( e.getErrCode(), spdlog::level::err, "Timeout on License request after {} attempts", lic_attempt );
                }
                if ( e.getErrCode() != DRM_WSMayRetry ) {
                    throw;
                }
    ...
    void setLicense( const Json::Value& license_json ) {
        ...
        try {
            ...
            if ( e.getErrCode() != DRM_BadFormat )
                throw;
            Throw( DRM_WSRespError, spdlog::level::err, "Malformed response from License Web Service: {}", e.what() );
        }
        ...
        if ( mLicenseCounter == 0 ) {
            ...
            if ( activationErrorCode ) {
                Throw( DRM_CtlrError, spdlog::level::err, "Failed to activate license on DRM controller, activationErr: 0x{:x}",
                      activationErrorCode );
            }
        ...
        if ( !isNodeLockedMode() ) {
            if ( !is_metered )
                Throw( DRM_CtlrError, spdlog::level::err, "DRM Controller failed to switch to Metering license mode" );
            else
                Debug( "DRM Controller is in Metering license mode" );
        } else {
            if ( !is_nodelocked )
                Throw( DRM_CtlrError, spdlog::level::err, "DRM Controller failed to switch to Node-Locked license mode" );
            else
    ...
    void installNodelockedLicense() {
        ...
        if ( isFile( mNodeLockLicenseFilePath ) ) {
            try {
                ...
                Throw( e.getErrCode(), spdlog::level::err, "Invalid local license file {} because {}. "
            ...
            try {
                ...
                Throw( e.getErrCode(), spdlog::level::err, "Failed to request license file: {}", e.what() );
            }
        }
        /// Install the license
    ...
    void detectDrmFrequencyMethod1() {
        ...
        if ( counter == 0xFFFFFFFF )
            Throw( DRM_BadFrequency, spdlog::level::err, "Frequency auto-detection failed: frequency_detection_period parameter ({} ms) is too long.",
    ...
    void checkDrmFrequency( int32_t measuredFrequency ) {
        ...
        if ( precisionError >= mFrequencyDetectionThreshold ) {
            Throw( DRM_BadFrequency, spdlog::level::err,
    ...
    ParameterKey findParameterKey( const std::string& key_string ) const {
        ...
        Throw( DRM_BadArg, spdlog::level::err, "Cannot find parameter: {}", key_string );
    }
    ...
    std::string findParameterString( const ParameterKey key_id ) const {
        ...
        if ( it == mParameterKeyMap.end() )
            Throw( DRM_BadArg, spdlog::level::err, "Cannot find parameter with ID: {}", key_id );
        return it->second;
    }
    ...
    void activate( const bool& resume_session_request = false ) {
        TRY
            ...
            if ( isDrmCtrlInNodelock() ) {
                Throw( DRM_BadUsage, spdlog::level::err, "DRM Controller is locked in Node-Locked licensing mode: "
        ...
        TRY
            for( const std::string& key_str : json_value.getMemberNames() ) {
                ...
                switch( key_id ) {
                    ...
                    default: {
                        Throw( DRM_BadArg, spdlog::level::err, "Parameter '{}' cannot be read", key_str );
                        break;
                    }
                }
            }
        ...
        TRY
            for( Json::ValueConstIterator it = json_value.begin() ; it != json_value.end() ; it++ ) {
                ...
                switch( key_id ) {
                    ...
                    case ParameterKey::mailbox_data: {
                        if ( !(*it).isArray() )
                            Throw( DRM_BadArg, spdlog::level::err, "Value must be an array of integers" );
                    ...
                    case ParameterKey::log_message_level: {
                        ...
                        if ( ( message_level < spdlog::level::trace)
                          || ( message_level > spdlog::level::off) )
                            Throw( DRM_BadArg, spdlog::level::err, "log_message_level ({}) is out of range [{:d}:{:d}]",
                    ...
                    default:
                        Throw( DRM_BadArg, spdlog::level::err, "Parameter '{}' cannot be overwritten", key_str );
                }
            }
        CATCH_AND_THROW
    }
