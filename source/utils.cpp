...
void saveJsonToFile( const std::string& file_path,
    ...
    if ( !ofs.is_open() )
        Throw( DRM_ExternFail, spdlog::level::err, "Unable to access file: {}", file_path );
    if ( writer->write( json_value, &ofs ) )
        Throw( DRM_ExternFail, spdlog::level::err, "Unable to write file: {}", file_path );
    if ( !ofs.good() )
        Throw( DRM_ExternFail, spdlog::level::err, "Unable to write file: {}", file_path );
    ofs.close();
}
...
Json::Value parseJsonString( const std::string &json_string ) {
    ...
    if ( json_string.size() == 0 )
        Throw( DRM_BadFormat, spdlog::level::err, "Cannot parse an empty JSON string" );

    if ( !reader->parse( json_string.c_str(), json_string.c_str() + json_string.size(),
            &json_node, &parseErr) )
        Throw( DRM_BadFormat, spdlog::level::err, "Cannot parse JSON string '{}' because {}", json_string, parseErr );

    if ( json_node.empty() || json_node.isNull() )
        Throw( DRM_BadArg, spdlog::level::err, "JSON string is empty" );
...
Json::Value parseJsonFile( const std::string& file_path ) {
    ...
    if ( !isFile(file_path) ) {
        Throw( DRM_BadArg, spdlog::level::err, "Path is not a valid file: {}", file_path );
    }

    // Open file
    std::ifstream fh( file_path );
    if ( !fh.good() )
        Throw( DRM_BadArg, spdlog::level::err, "Cannot find JSON file: {}", file_path );
    // Read file content
    ...
    try {
        ...
        Throw( e.getErrCode(), spdlog::level::err, "Cannot parse JSON file {}: {}", file_path, e.what() );
    }
...
const Json::Value& JVgetRequired( const Json::Value& jval,
    ...
    if ( !jval.isMember( key ) )
        Throw( DRM_BadFormat, spdlog::level::err, "Missing parameter '{}' of type {}", key, typeToString( type ) );

    const Json::Value& jvalmember = jval[key];

    if ( ( jvalmember.type() != type ) && !jvalmember.isConvertibleTo( type ) )
        Throw( DRM_BadFormat, spdlog::level::err, "Wrong parameter type for '{}' = {}, expecting {}, parsed as {}",
              key, jvalmember.toStyledString(), typeToString( type ), typeToString( jvalmember.type() ) );

    if ( jvalmember.empty() )
        Throw( DRM_BadFormat, spdlog::level::err, "Value of parameter '{}' is empty", key );

    if ( ( jvalmember.type() == Json::stringValue ) && ( jvalmember.asString().empty() ) )
        Throw( DRM_BadFormat, spdlog::level::err, "Value of parameter '{}' is an empty string", key );
