...
@pytest.mark.awsxrt
def test_vitis_10activator_200_125(accelize_drm, conf_json, cred_json, async_handler, log_file_factory):
    """
    Test a vitis configuration: 10 activators in dual clock
    """
    # Run test
    design_name = 'vitis_10activator_200_125'
    axiclk_freq_ref = 219
    drmclk_freq_ref = 87
    log_content = run_test_on_design(accelize_drm, design_name, conf_json, cred_json, async_handler,
                                    log_file_factory, axiclk_freq_ref, drmclk_freq_ref)
...
@pytest.mark.awsxrt
def test_vitis_1activator_twice_200_125(accelize_drm, conf_json, cred_json, async_handler, log_file_factory):
    """
    Test a vitis configuration with 2 instances of the same activator
    """
    # Run test
    design_name = 'vitis_1activator_twice_200_125'
    axiclk_freq_ref = 219
    drmclk_freq_ref = 125
    log_content = run_test_on_design(accelize_drm, design_name, conf_json, cred_json, async_handler,
                                    log_file_factory, axiclk_freq_ref, drmclk_freq_ref)
