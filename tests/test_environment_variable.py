...
def test_env_var_DRMSAAS_URL(accelize_drm, conf_json, cred_json, async_handler,
    ...
    assert search(r'Use environment variable DRMSAAS_URL: %s' % environ['DRMSAAS_URL'],
...
def test_env_var_DRMSAAS_CLIENT_ID(accelize_drm, conf_json, cred_json, async_handler,
    ...
    async_cb.assert_NoError()
    log_content = logfile.read()
    assert search(r'Use environment variable DRMSAAS_CLIENT_ID: %s' % environ['DRMSAAS_CLIENT_ID'],
                log_content, IGNORECASE)
...
def test_env_var_DRMSAAS_CLIENT_SECRET(accelize_drm, conf_json, cred_json, async_handler,
    ...
    async_cb.assert_NoError()
    log_content = logfile.read()
    assert search(r'Use environment variable DRMSAAS_CLIENT_SECRET', log_content, IGNORECASE)
