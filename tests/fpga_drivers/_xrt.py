...
class FpgaDriver(_FpgaDriverBase):
    ...
    def _get_driver():
        ...
        if _isfile(_join(xrt_path, 'lib','libxrt_core.so')):
            print('Loading XRT API library for Xilinx XRT targets')
    ...
    def _program_fpga(self, fpga_image):
        ...
        load_image = _run(
            ...
            stderr=_STDOUT, stdout=_PIPE, universal_newlines=True, check=False)
        if load_image.returncode or not search(r'program succeeded', load_image.stdout):
    ...
    def _reset_fpga(self):
        ...
        """
        self._program_fpga(self._fpga_image)
