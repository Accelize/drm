...
@pytest.mark.skip(reason='Temporary test to reproduce metering data corruption issue')
def test_tmp_metered_pause_resume_long_time(accelize_drm, conf_json, cred_json, async_handler):
    """
    Test no error occurs in normal start/stop metering mode during a long period of time
    """
    driver = accelize_drm.pytest_fpga_driver[0]
    ...
    for r in range(nb_run):
        ...
        #            random_wait = int(lic_duration*1.5)
                    wait_deadline(start, random_wait)
                    drm_manager.activate(True)
                    start = datetime.now()
                except:
